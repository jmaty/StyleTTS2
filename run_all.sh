...
# -----------------------------------------------------------------------------
if [[ "$#" -lt 2 ]]; then
     echo "Usage: run_all.sh settings.yml config.yml" >&2
     exit 1
fi
SETTINGS=$1  # Training stages settings file
CONFIG=$2  # Experiment config file

# Input experimental directory
EXPDIR=$(dirname "$SETTINGS")
...
./Bin/prep_configs.sh $SETTINGS $CONFIG
...
jobid1b=$(./run_stage1.sh $EXPDIR/${CONFIG_NAME}1b.yml $queue $hours $ngpus 2>/dev/null)

# Run stage 2a
ngpus=$(yq '.start2.ngpus' "$SETTINGS")
[[ $ngpus -gt 2 ]] && queue=dgx || queue=gpu4
hours=$(yq '.start2.hours' "$SETTINGS")
jobid2a=$(./run_stage2.sh $EXPDIR/${CONFIG_NAME}2a.yml $queue $hours $ngpus 2>/dev/null)

# Run stage 2b
ngpus=$(yq '.diff.ngpus' "$SETTINGS")
[[ $ngpus -gt 2 ]] && queue=dgx || queue=gpu4
hours=$(yq '.diff.hours' "$SETTINGS")
jobid2b=$(./run_stage2.sh $EXPDIR/${CONFIG_NAME}2b.yml $queue $hours $ngpus 2>/dev/null)

# Run stage 2c
ngpus=$(yq '.joint.ngpus' "$SETTINGS")
[[ $ngpus -gt 2 ]] && queue=dgx || queue=gpu4
hours=$(yq '.joint.hours' "$SETTINGS")
jobid2c=$(./run_stage2.sh $EXPDIR/${CONFIG_NAME}2c.yml $queue $hours $ngpus 2>/dev/null)
