...
def main():
    ...
    test_audio_dir = os.path.join(config['log_dir'],
                                  config['data_params'].get('test_audio_dir', 'test_audios'))
    n_val_samples = data_params.get('n_val_samples', 6)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if accelerator.is_main_process:
            ...
            curr_loss_test = loss_test / iters_test
            log_print(f'Epoch [{epoch+1:3}/{epochs}]: validation loss: {curr_loss_test:.3f}', logger)
            # print('\n\n\n')
            ...
            with torch.no_grad():
                for idx, (m, w) in enumerate(zip(mel_input_length, waves)):
                    ...
                    # Use up to the defined validation samples
                    if idx >= n_val_samples:
                        break
